...
public abstract class CanonBase extends PtpCamera implements FullCamera {
	...
	protected void cmd_fetchAllCameraProperties() {
		...
		if (evtData.getStatusChanged())
		{
//			prop_batteryLevel = evtData.getBatteryLevel();
//			prop_exposureIndex = decode_exposureIndex(evtData.getExposureIndex());
			prop_exposureMode = decode_exposureMode(evtData.getExposureMode());
//			prop_exposureTime = decode_exposureTime(evtData.getExposureTime());
//			prop_fNumber = decode_fNumber(evtData.getFNumber());
//			prop_focusMode = decode_focusMode(evtData.getFocusMode());
		}
	}
...
/*		Arr<ObjectFormatCode> formats = getDeviceInfo().getCaptureFormats();
	...
	private ExposureMode decode_exposureMode(UInt32 in) {
		String name = null;
		switch (in.getValue()) {
			case (short) 0x0000:
				name = "Program";
		...
		return new ExposureMode(name, (short)in.getValue());
	}
